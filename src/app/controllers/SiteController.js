...
class SiteController {
    ...
    async home(req, res, next) {
        await Product.find({})
            ...
            .then(product => {
                ...
                res.render('home', { products: mutipleMongooseToObj(product), showFooter } )
            })
            .catch(err => next(err))
    }
    ...
    async product(req, res, next) {
        await Product.find({})
            ...
            .then(product => {
                const showFooter = true;
                res.render('shop', { products: mutipleMongooseToObj(product), showFooter } )
            })
            .catch(err => next(err))
    }
    ...
    blog(req, res, next) {
        const showFooter = true;
        res.render('blog', { showFooter})
    }

    about(req, res, next) {
        const showFooter = true;
        res.render('about', { showFooter} )
    }

    contact(req, res, next) {
        const showFooter = true;
        res.render('contact', { showFooter})
    }

    dashboard(req, res, next) {
        const showHeader = true;
        res.render('admin/dashboard', { showHeader })
    }
